...
class WaterIntakeBloc extends Bloc<WaterIntakeEvent, WaterIntakeState> {
  ...
  Stream<WaterIntakeState> mapEventToState(
    ...
    try {
      if (currentState.getDay() !=
          DateTime(DateTime.now().day).toIso8601String()) {
        yield UnWaterIntakeState(0);
      }
      ...
      if (event is FetchWaterIntakeEvent) {
       AutogeneratedWaterIntake result = await _waterIntakeRepository.fetchWaterIntake(
            uid, currentState.getDay());

        yield InWaterIntakeState(currentState.version + 1,
            int.parse(result.results[0].quantity_in_ml),result.results[0].day);
      }
      if (event is AddtwoFiftyMlevent) {
        var update_value = (currentState.getCurrentQuantityInMl() + 250 > 3800)
            ? 3800
            : currentState.getCurrentQuantityInMl() + 250;
        yield InWaterIntakeState(
            currentState.version + 1, update_value, currentState.getDay());
      }
